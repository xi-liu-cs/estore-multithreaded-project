...
class EStore {
    ...
    smutex_t mutex;
    scond_t ready; // ready to buy, !(not in stock || cost > budget)
    double store_discount = 0.0; 
    double ship_cost = 3.0;  //flat overall store shipping fee
    smutex_t mutex_array[INVENTORY_SIZE];
    ...
    double cost_item(Item * item_p);

    void handle_mode(int item_id);
    void handle_mode_end(int item_id);
};
